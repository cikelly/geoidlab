...
from ...src.easy_pygeoid.utils import compute_tc_chunk
from numpy.lib.stride_tricks import sliding_window_view
import bottleneck as bn
from joblib import Parallel, delayed
from tqdm.contrib.concurrent import process_map

def wrapper(args) -> tuple[int, int, np.ndarray]:
    row_start, row_end, ncols_P, dm, dn, lamp, phip, Hp, ori_topo, X, Y, Z, Xp, Yp, Zp, radius, G_rho_dxdy = args
    return compute_tc_chunk(
        row_start, row_end, ncols_P, dm, dn, lamp, phip, Hp,
        ori_topo, X, Y, Z, Xp, Yp, Zp, radius, G_rho_dxdy
    )
        ...
        for i in tqdm(range(nrows_P), desc='Computing terrain correction'):
            ...
            for j in range(ncols_P):
                ...
                x = coslamp[i, j] * (smallY - self.Yp[i, j]) - \
                    sinlamp[i, j] * (smallX - self.Xp[i, j])
                y = cosphip[i, j] * (smallZ - self.Zp[i, j]) - \
                    coslamp[i, j] * sinphip[i, j] * (smallX - self.Xp[i, j]) - \
                    sinlamp[i, j] * sinphip[i, j] * (smallY - self.Yp[i, j])
    ...
    def terrain_correction_parallel(self, chunk_size: int = 10, progress=True) -> np.ndarray:
        ...
        if progress:
            ...
            args = [
                ...
                self.radius, G_rho_dxdy) for row_start, row_end in chunks
            ]
            
            # Use process_map for parallel processing with progress bar
            results = process_map(wrapper, args, desc='Computing terrain correction')
        else:
